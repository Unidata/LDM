...
RawSocketComm::RawSocketComm(const char* if_name) {
	...
	send_frame.proto = htons(FMTP_PROTO_TYPE);
}
...
ssize_t RawSocketComm::SendPacket(PacketBuffer* buffer, int flags, void* dst_addr) {
	...
	eth_header->h_proto = htons(FMTP_PROTO_TYPE);

	return SendFrame(buffer->eth_header, buffer->data_len + FMTP_HLEN + ETH_HLEN);
}
...
bool RawSocketComm::IsMyPacket() {
	if (memcmp(recv_frame.dst_addr, bind_mac_addr, 6) == 0
		&& recv_frame.proto == htons(FMTP_PROTO_TYPE)) {
		return true;
	}
	else
		return false;

...
SenderStatusProxy::SenderStatusProxy(string addr, int port, string group_addr, int fmtp_port, int buff_size)
...
void SenderStatusProxy::InitializeExecutionProcess() {
	...
	ptr_sender = new FMTPSender(buffer_size);
	ptr_sender->SetStatusProxy(this);
	ptr_sender->JoinGroup(fmtp_group_addr, fmtp_port_num);

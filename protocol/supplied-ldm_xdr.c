...
bool_t
xdr_ServiceAddr (XDR *xdrs, ServiceAddr *objp)
{
	register int32_t *buf;

	 if (!xdr_string (xdrs, &objp->inetId, ~0))
		 return FALSE;
	 if (!xdr_u_short (xdrs, &objp->port))
		 return FALSE;
	return TRUE;
...
bool_t
xdr_SubnetLen (XDR *xdrs, SubnetLen *objp)
{
	register int32_t *buf;
...
bool_t
xdr_CidrAddr (XDR *xdrs, CidrAddr *objp)
{
	register int32_t *buf;

	 if (!xdr_in_addr_t (xdrs, &objp->addr))
		 return FALSE;
	 if (!xdr_SubnetLen (xdrs, &objp->prefixLen))
		 return FALSE;
	return TRUE;
}
...
bool_t
xdr_VlanId (XDR *xdrs, VlanId *objp)
{
	register int32_t *buf;
...
bool_t
xdr_VcEndPoint (XDR *xdrs, VcEndPoint *objp)
{
	register int32_t *buf;

	 if (!xdr_string (xdrs, &objp->switchId, ~0))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->portId, ~0))
		 return FALSE;
	 if (!xdr_VlanId (xdrs, &objp->vlanId))
		 return FALSE;
	return TRUE;
}
...
bool_t
xdr_McastSubReq (XDR *xdrs, McastSubReq *objp)
{
	register int32_t *buf;

	 if (!xdr_feedtypet (xdrs, &objp->feed))
		 return FALSE;
	 if (!xdr_VcEndPoint (xdrs, &objp->vcEnd))
		 return FALSE;
	return TRUE;
}

bool_t
xdr_McastProdIndex (XDR *xdrs, McastProdIndex *objp)
{
	register int32_t *buf;
...
xdr_MissedProduct (XDR *xdrs, MissedProduct *objp)
{
	register int32_t *buf;
	 ...
	 if (!xdr_McastProdIndex (xdrs, &objp->iProd))
		 return FALSE;
...
bool_t
xdr_BacklogSpec (XDR *xdrs, BacklogSpec *objp)
{
	register int32_t *buf;

	 if (!xdr_signaturet (xdrs, objp->after))
		 return FALSE;
	 if (!xdr_int (xdrs, &objp->afterIsSet))
		 return FALSE;
	 if (!xdr_signaturet (xdrs, objp->before))
		 return FALSE;
	 if (!xdr_u_int (xdrs, &objp->timeOffset))
		 return FALSE;
	return TRUE;
}
...
bool_t
xdr_McastInfo (XDR *xdrs, McastInfo *objp)
{
	register int32_t *buf;
	 ...
	 if (!xdr_string (xdrs, &objp->group, ~0))
		 return FALSE;
	 if (!xdr_string (xdrs, &objp->server, ~0))
		 return FALSE;
	return TRUE;
}

bool_t
xdr_McastSubInfo (XDR *xdrs, McastSubInfo *objp)
{
	register int32_t *buf;

	 if (!xdr_McastInfo (xdrs, &objp->mcastInfo))
		 return FALSE;
	 if (!xdr_CidrAddr (xdrs, &objp->fmtpAddr))
		 return FALSE;
	return TRUE;
}
...
xdr_SubscriptionReply (XDR *xdrs, SubscriptionReply *objp)
{
	...
	switch (objp->status) {
	case LDM7_OK:
		 if (!xdr_McastSubInfo (xdrs, &objp->SubscriptionReply_u.info))
			 return FALSE;
		break;
	case LDM7_UNAUTH:
		break;

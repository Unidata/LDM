        ...
        if (fcntl(fd, cmd, &lock) < 0)
        {
                ...
                if(errnum == EDEADLK || errnum == EAGAIN || errnum == EACCES)
                {
                        ...
                        log_errno(errnum,
                                ...
                                strerror(errnum));
                        log_error("conflicting pid %d", (int)conflict);
                        }
                }
                else
                {
        ...
        void **const ptrp)
{
        ...
        {
                status = errno;
                log_syserr("f_ftom malloc %lu", (unsigned long)extent);
                return status;
        }
        /* DEBUG */
        memset(vp, 0, extent);
        ...
        {
                status = errno;
                log_syserr("f_ftom lseek %ld", (long)offset);
                goto unwind_lock;
        }
        ...
        if(nread != extent)
        {
                ...
                if(nread == -1)
                {
                        log_syserr("f_ftom read %lu", (unsigned long)extent);
                        goto unwind_lock;
                }
                /* else */
                if(status != ENOERR)
                {
                        log_syserr("f_ftom at %ld incomplete read %d != %lu",
        ...
        int const rflags)
{
        ...
        if(fIsSet(rflags, RGN_MODIFIED))
        {
                ...
                {
                        status = errno;
                        log_syserr("f_mtof lseek %ld", (long)offset);
                        goto unwind_vp;
                }
                ...
                if(nwrote != extent)
                {
                        ...
                        if(nwrote == -1)
                        {
                                log_syserr("f_mtof write %lu",
                                        (unsigned long)extent);
                        }
                        else
                        {
                        log_syserr("f_mtof at %ld incomplete write %d != %lu",
                                        (long)offset, nwrote,
                                        (unsigned long)extent);
                        }
                }
    ...
    const size_t        maxProds)
{
...

    {
        ...
        if (pthread_mutexattr_init(&mutexAttr)) {
            log_notice("Couldn't initialize product-queue mutex attributes");
            free(pq);
            return NULL;
        }
        else {
            ...
            if (pthread_mutex_init(&pq->mutex, &mutexAttr)) {
                log_notice("Couldn't initialize product-queue mutex");
                free(pq);
                return NULL;
            }
...
ctl_rel(pqueue *const pq, const int rflags)
{
        ...
        if(pq->ixp != NULL)
        {
                ...
                if(status != ENOERR)
                        log_error("mtof ix: %s", strerror(status));
                pq->ixp = NULL;
                pq->rlp = NULL;
                pq->tqp = NULL;
                pq->sxp = NULL;
        ...
        if(pq->ctlp != NULL)
        {
                ...
                if(status != ENOERR)
                        log_error("mtof ctl: %s", strerror(status));
                pq->ctlp = NULL;
        }
...
remap:
        ...
        {
                /* Not a product queue */
                log_error("%s: Not a product queue\n", path);
                status = EINVAL;
                goto unwind_map;
        }
    ...
    prod_info* const restrict  info)
{
    ...
    if (TV_CMP_LT(*creationTime, now)) {
        ...
        if (tvIsNone(pq->ctlp->minVirtResTime) ||
            ...
            log_info("pq_set_mvrt(): MVRT product: %s",
    ...
    if (status == ENOERR) {
         ...
         */
        {
            ...
            if (TV_CMP_LT(*creationTime, now)) {
                ...
                if (tvIsNone(pq->ctlp->minVirtResTime) || 
                        (TV_CMP_LT(virtResTime, pq->ctlp->minVirtResTime)))  {
                    log_info("pq_del_oldest(): MVRT product: %s",
                ...
                if (!fIsSet(pflags, PQ_READONLY)) {
                    ...
                    if (!status) {
                        ...
                        if (MAX_WRITE_COUNT > ctlp->write_count) {
                            ...
                            log_error("Too many writers (%u) to product-queue "
    ...
    const signaturet    signature)
{
    ...
    status = ctl_get(pq, 0);
    if (ENOERR != status) {
        log_syserr("Couldn't lock control-region "
            "of product-queue");
    }
    else {
        tqelem* timeEntry;
        ...
        off_t* const restrict        off)
{
        ...
        if(log_is_enabled_debug) {
            ...
            log_debug("pq_sequence(): time(insert)-time(create): %.4f s", latency);
        }
        ...
        timestampt *tsp) /* modified upon return */
{
                        ...
                        == ENOERR) 
        {
           ...
           {
                log_debug("cursor reset: stop searching\0");
                unlockIf(pq);
                return status;
           }
        }

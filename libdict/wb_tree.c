 ...
 *
 * $Id: wb_tree.c,v 1.1.2.1 2009/08/14 14:34:24 steve Exp $
 *
 ...
 * unsimplified to make it more obvious how those values were derived.
 *
	...
	int			 overwrite;
{
	...
	while ((node = parent) != NULL) {
		parent = node->parent;
		node->weight++;
		wbal = WEIGHT(node->llink) / (double)node->weight;
		if (wbal < ALPHA_0) {
			wbal = WEIGHT(node->rlink->llink) / (double)node->rlink->weight;
			if (wbal < ALPHA_3) {		/* LL */
				rot_left(tree, node);
			} else {					/* RL */
			...
			wbal = WEIGHT(node->llink->llink) / (double)node->llink->weight;
			if (wbal > ALPHA_2) {		/* RR */
				rot_right(tree, node);
			} else {					/* LR */

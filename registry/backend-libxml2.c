 ...
 * Returns
 *      0               Success.
 *      EIO             I/O failure. "log_start()" called.
 *      ENOMEM          System failure. "log_start()" called.
 */
static RegStatus
fileNew(
    const char*         path,
    ...
    File** const        file)
{
    ...
    if (NULL == f) {
        LOG_START1("Couldn't allocate %lu bytes", sizeof(File));
        status = ENOMEM;
    }
    else {
        f->isLocked = 0;
        ...
        if (NULL == f->path) {
            LOG_SERROR1("Couldn't duplicate string \"%s\"", path);
            status = ENOMEM;
            free(f);
        }
        else {
 ...
 *      EIO             I/O error. "log_start()" called.
 */
static RegStatus
fileGetSize(
    const File* const   file,
    off_t* const        size)
{
    ...
    if (fstat(file->fd, &stat) == -1) {
        LOG_SERROR1("Couldn't fstat(2) file \"%s\"", file->path);
        return EIO;
    }
 ...
 * Returns
 *      0               Success
 *      EIO             I/O error. "log_start()" called. State of "file" is
 *                      unchanged.
 */
static RegStatus
fileLock(
    File* const file)
{
    ...
    if (file->isLocked) {
        ...
        if (-1 == fd) {
            LOG_SERROR2("Couldn't open file \"%s\" for %s", 
            ...
            if (fcntl(fd, F_SETLKW, &flock) == -1) {
                LOG_SERROR1("Couldn't lock file \"%s\"", file->path);
                status = EIO;
            }
            else {
                file->fd = fd;
    ...
    File* const file)
{
    ...
    if (file->isLocked) {
        ...
        if (fcntl(file->fd, F_SETLK, &flock) == -1) {
            LOG_SERROR1("Couldn't unlock file \"%s\"", file->path);
            status = EIO;
        }
        else {
            file->isLocked = 0;
        ...
        if (close(file->fd) == -1) {
            LOG_SERROR1("Couldn't close file \"%s\"", file->path);
            status = EIO;
        }
    }
 ...
 * Returns
 *      0               Success
 *      EIO             I/O failure. "log_start()" called. "file" is unchanged.
 */
static RegStatus
fileFree(
    File* const file)
 ...
 * Returns
 *      0               Success
 *      EACCES          The file isn't exclusively locked. "log_start()" called.
 *                      "file" is unchanged.
 *      EIO             I/O failure. "log_start()" called.  "file" is unchanged.
 */
static RegStatus
    ...
    File* const file)
{
    ...
    if (!(file->isLocked && file->exclusive)) {
        LOG_START1("File \"%s\" isn't exclusively locked. Can't delete.",
                file->path);
        status = EACCES;
    }
    else {
        if (unlink(file->path) == -1) {
            LOG_SERROR1("Couldn't unlink(2) file \"%s\"", file->path);
            status = EIO;
        }
        else {
    ...
    char** const        path)
{
    ...
    if (NULL == p) {
        LOG_SERROR1("Couldn't allocate %lu bytes for pathname", nbytes);
        status = ENOMEM;
    }
    else {
    ...
    void*               arg)
{
    ...
    if (NULL == dupKey) {
        LOG_SERROR1("Couldn't duplicate string \"%s\"", key);
        status = ENOMEM;
    }
    else {
        char*           lasts;
    ...
    Backend* const      backend)
{
    ...
    if (backend->forWriting && backend->modified) {
        ...
        if (xmlSaveFormatFile(path, backend->doc, 1) == -1) {
            LOG_SERROR1("Couldn't write XML file \"%s\"", path);
            status = EIO;
        }
 ...
 * Returns
 *      0               Success
 *      ENOMEM          Out-of-memory error. "log_start()" called.
 */
static RegStatus
    ...
    Backend*    back)
{
    ...
    if (0 < nodeCount) {
        ...
        if (NULL == index) {
            LOG_SERROR1("Couldn't allocate %d elements for sorted index",
                    nodeCount);
            status = ENOMEM;
        }
        else {
    ...
    Backend* const      back)
{
    ...
    if (NULL == doc) {
        LOG_SERROR0("Couldn't create new XML document");
        status = ENOMEM;
    }
    else {
        ...
        if (NULL == rootNode) {
            LOG_SERROR0("Couldn't create root-node of XML document");
            status = ENOMEM;
        }
        else {
    ...
    const char* const   path)
{
    ...
    if (NULL == doc) {
        LOG_START1("Couldn't parse XML file \"%s\"", path);
        status = EIO;
    }
    else {
        ...
        if (NULL == rootNode) {
            LOG_START1("XML file \"%s\" is empty", path);
            status = EIO;
        }
        else {
            back->doc = doc;
 ...
 * RETURNS:
 *      0               Success.
 *      EIO             Backend database error.  "log_start()" called. "*back"
 *                      is unchanged.
 *      ENOMEM          System error.  "log_start()" called. "*back" is 
 *                      unchanged.
 */
static RegStatus
acquireBackend(
    ...
    int                 forWriting)
{
    ...
    if (NULL == back) {
        LOG_SERROR1("Couldn't allocate %lu bytes", (long)sizeof(Backend));
        status = ENOMEM;
    }
    else {
        char*   path;
    ...
    const char* const   value)
{
    ...
    if (strchr(key, ' ') != NULL) {
        LOG_START1("Key \"%s\" has a space", key);
        status = EINVAL;
    }
    else {
        char* const dupKey = strdup(key);
    ...
    const Backend* const        backend)
{
    ...
    if (NULL == content) {
        LOG_SERROR1("Couldn't get value of key \"%s\"", key);
    }
    else {
        if (sb_set(backend->content, content, NULL)) {
            LOG_ADD1("Couldn't get value of key \"%s\"", key);
        }
        else {

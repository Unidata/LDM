...
int main()
{
    ...
    log_assert(-1 != (pid = fork()));

    if (0 != pid) {
        int childStatus;

        log_assert(pid == wait(&childStatus));
        ...
        log_assert(NULL != (hp = gethostbyname("emo.unidata.ucar.edu")));
        ...
        log_assert(NULL != (clnt = clnttcp_create(&addr, 300029, 6, &sock1, 0, 0)));

        (void)fprintf(stderr, "sock1 = %d\n", sock1);

        log_assert(-1 != (flags = fcntl(sock1, F_GETFL)));
        ...
        log_assert(-1 != (flags = fcntl(sock2, F_GETFL)));

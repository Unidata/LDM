 ...
 * Receives data.
 *
 * @param[in] sock             The communications socket with the remote LDM-7.
 ...
 * @param[in] sock           The socket used to communicate with the server.
 * @param[in] mcastName      The name of the multicast group.
 * @retval    0              Success.
    ...
    const char* const restrict mcastName)
{
	...
	status = clntStatusToLdm7Status(clnt_stat(clnt));
        ...
        if (reply->status == 0) {
            status = execute(sock, &reply->SubscriptionReply_u.groupInfo,
                    missedProdFunc);
        }

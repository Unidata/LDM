    ...
    size_t      max)
{
    ...
    if (start == (off_t)-1) {
        ...
        if (fscanf(file, "{%d,", &len) != 1) {
            ...
            if (fgetc(file) != '}') {
                log_add("scanString(): String isn't terminated");
            }
            else {
    ...
    FILE*               file)
{
    ...
    if (start == (off_t)-1) {
        ...
        if (fscanf(file, "{%80[^,]", buf) != 1 ||
            ...
            log_add_syserr("Couldn't scan product feedtype");
        }
        else if (fscanf(file, ",%u", &info->seqno) != 1) {
            log_add_syserr("Couldn't scan product sequence-number");
        }
        else if (fgetc(file) != ',' ||
            ...
            log_add_syserr("Couldn't scan product size");
        }
        else {
            if (fgetc(file) != ',' ||
                ...
                if (fgetc(file) != ',' ||
                    ...
                    if (fgetc(file) != '}') {
                        log_add("pi_scan(): Information isn't terminated");
                    }
                    else {
                        nbytes = (int)(ftello(file) - start);
                    }

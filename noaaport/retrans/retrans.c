		...
		int rqst_cause)
{
	...
	if(global_retransmitpipe_fd <= 0){
		...
		uerror("Unable to open or write to pipe %s \n",DEFAULT_RETRANSMIT_PIPENAME);
		return (0);
	}
	...
	p_pipe_requesthdr->pipe_request_cause = RETRANS_RQST_CAUSE_RCV_ERR;
        ...
        if(ulogIsDebug()){
            udebug("pipe_request_numb = %ld | ctl_flag = %d | link_id = %d | channel_type = %d | rqst cause = %d \n", 
            ...
            udebug("cpio addr = %ld | rqst time = %ld | first prod seqno = [%d-%ld] | last prod seqno = %ld | run number = %d | delay_send = %d \n",
	...
	if((bytes_written = write(global_retransmitpipe_fd,
		...
		uerror("FAIL write#%ld pipe[%d] tot(%ld) %s link[%d] prod(%ld-%ld)\n",
		...
		if(first_prod_seqno != last_prod_seqno) {
			uinfo("OK rqst#%ld tot(%ld) %s link[%d] prod(%ld-%ld)\n",
				request_numb,
				p_acqtable->proc_tot_prods_retrans_rqstd, 
				GET_SBN_TYP_NAME(p_pipe_requesthdr->pipe_channel_type),
				p_pipe_requesthdr->pipe_link_id,
			...
			uinfo("OK rqst#%ld tot(%ld) %s link[%d] prod(%ld)\n",
				request_numb,
				p_acqtable->proc_tot_prods_retrans_rqstd, 
				GET_SBN_TYP_NAME(p_pipe_requesthdr->pipe_channel_type),
				p_pipe_requesthdr->pipe_link_id,
...
/* Debug only */
	if(ulogIsDebug()) {
		if(p_buffhdr != (BUFF_HDR *)NULL) {
	    	udebug("%s %s prod(%ld) code=%d %s[0x%x] update\n",
				FNAME,
				GET_PROD_TYPE_NAME(p_buffhdr->proc_prod_type),
				prod_seqno,
				p_buffhdr->proc_prod_code,
	    	...
	    	udebug("%s prod(%ld)  %s[0x%x] update\n",
				FNAME,
				prod_seqno,
				(entry_flag & RETRANS_ENTRY_FLAG_AVAIL)?"AVAIL":
				(entry_flag & RETRANS_ENTRY_FLAG_NEW_VALID)?"NEW_VALID":
		...
		if(prod_errors < 0){
			prod_errors = 0;
				...
				if(prod_errors > 0){
					...
					if(p_acqtable->proc_orig_prod_seqno_last != 0){
                                           if(ulogIsDebug())
						udebug(" Aborting orig seqno [%ld] in retrans table. Cuurent prod seqno [%ld] \n",
